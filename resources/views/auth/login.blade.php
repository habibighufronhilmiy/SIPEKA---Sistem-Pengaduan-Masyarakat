...
<body class="min-h-screen flex items-start justify-center auth-grad relative overflow-y-auto py-6">
    ...
    <div class="w-full max-w-md px-4">
        ...
        <div class="bg-white/95 backdrop-blur-xl rounded-3xl p-8 shadow-2xl">
            <h2 class="text-2xl font-extrabold text-gray-900 text-center mb-1">Masuk Bro! 👋</h2>
            <p class="text-gray-500 text-center text-sm mb-6">Yuk lanjutin perjuanganmu</p>

            <form method="POST" action="{{ route('login') }}">
                @csrf
                <div class="mb-4">
                    <label class="block text-sm font-semibold text-gray-700 mb-1.5">Email atau Username</label>
                    <input type="text" name="login" value="{{ old('login') }}" required autofocus
                        class="w-full px-4 py-3 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500 bg-gray-50/50 text-gray-900 placeholder:text-gray-400 transition">
                    @error('login')<p class="text-red-500 text-sm mt-1.5 flex items-center gap-1"><svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7 4a1 1 0 11-2 0 1 1 0 012 0zm-1-9a1 1 0 00-1 1v4a1 1 0 102 0V6a1 1 0 00-1-1z" clip-rule="evenodd"/></svg>{{ $message }}</p>@enderror
                </div>
                <div class="mb-2">
                    <label class="block text-sm font-semibold text-gray-700 mb-1.5">Password</label>
                    <div class="relative">
                        <input type="password" name="password" id="password" required
                            class="w-full px-4 py-3 pr-11 border border-gray-200 rounded-xl focus:ring-2 focus:ring-primary-500 focus:border-primary-500 bg-gray-50/50 text-gray-900 placeholder:text-gray-400 transition">
                ...
                <div class="text-right mb-6">
                    <a href="{{ route('password.request') }}" class="text-sm text-primary-600 font-bold hover:text-primary-700">Lupa Password?</a>
                </div>
            ...
            <p class="text-center mt-6 text-sm text-gray-500">
                Belum punya akun? <a href="{{ route('register') }}" class="text-primary-600 font-bold hover:text-primary-700">Daftar</a>
            </p>
        </div>
    </div>
    <script>
